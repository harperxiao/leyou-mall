...
//        MatchQueryBuilder basicQuery = QueryBuilders.matchQuery("all", request.getKey()).operator(Operator.AND);
        ...
        //解析分类聚合
        List<Map<String,Object>> categories = handleCategoryAgg(aggs.get(categoryAggName));
        //解析品牌聚合
    ...
    private List<Brand> handleBrandAgg(Aggregation aggregation) {
        ...
        try {
            ...
            return terms.getBuckets().stream().map(b -> {
                        return this.brandClient.queryBrandById(b.getKeyAsNumber().longValue());
...
//            log.error("查询品牌信息失败", e);
    ...
    private List<Map<String,Object>> handleCategoryAgg(Aggregation aggregation) {
        try {
            LongTerms terms = (LongTerms) aggregation;
